...
import { loginUser, loginWithGoogle, setStoredToken } from '../services/api.js'
...
export default function LoginForm() {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setErrorMessage('')
      try {
        // Exchange the Google authorization code / access token for a PrakritiAI JWT
    ...
    <div className="relative z-10 mx-auto w-full max-w-[440px] rounded-[30px] border border-black/5 bg-pa-cream-card px-6 py-6 shadow-[0_30px_80px_rgba(0,0,0,0.14)] sm:px-8 sm:py-7 md:px-9 md:py-8">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-xl border border-black/10 bg-pa-input-2 py-2 sm:py-2.5 font-sans text-sm font-semibold text-[#2a2a2a] transition hover:bg-[#e4e2d4] disabled:cursor-not-allowed disabled:opacity-70"
        >
